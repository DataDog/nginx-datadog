...
ngx_array_t* discover_span_context_keys(ngx_pool_t* pool, ngx_log_t* log,
  ...
  ngx_array_t* result = ngx_array_create(pool, tag_names.size(), sizeof(std::string_view));
  if (result == nullptr) {
    throw std::bad_alloc{};
  }

  for (const std::string_view& tag_name : tag_names) {
    const auto new_element = static_cast<std::string_view*>(ngx_array_push(result));
