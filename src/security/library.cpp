...
class FinalizedConfigSettings {
 ...
 public:
  ...
  std::optional<HashedLowercaseStringView> custom_ip_header() const {
    if (custom_ip_header_.empty()) {
      return std::nullopt;
    }
    return {{custom_ip_header_lowercase_key_, custom_ip_header_hash_}};
  };
 ...
 private:
  ...
  LowercaseStringView custom_ip_header_lowercase_key_;
...
FinalizedConfigSettings::FinalizedConfigSettings(
  ...
  custom_ip_header_lowercase_key_ = LowercaseStringView{
      custom_ip_header_, LowercaseStringView::WithRuntimeCheckTag{}};
  ...
  if (ngx_conf.api_security_proxy_sample_rate == NGX_CONF_UNSET) {
    auto maybe_sample_rate =
        get_env_unsigned(evs, "DD_API_SECURITY_PROXY_SAMPLE_RATE"sv);
  ...
  // Validation: warn if DD_API_SECURITY_ENABLED is true but
  // DD_API_SECURITY_PROXY_SAMPLE_RATE is 0
  if (api_security_enabled_ && api_security_proxy_sample_rate_ == 0) {
    ngx_log_error(NGX_LOG_WARN, ngx_cycle->log, 0,
                  "DD_API_SECURITY_ENABLED is true but "
                  "DD_API_SECURITY_PROXY_SAMPLE_RATE is 0. "
...
std::optional<ddwaf_owned_map> Library::initialize_security_library(
  ...
  if (conf.api_security_enabled()) {
    ...
    if (api_security_shm_zone_ != nullptr) {
      auto limiter_opt = ApiSecurityLimiterZone::get_limiter(
          api_security_shm_zone_, max_per_min);
      if (limiter_opt) {
        shared_api_security_limiter_ =
            std::make_unique<SharedApiSecurityLimiter>(*limiter_opt);
        ngx_log_error(
            NGX_LOG_INFO, ngx_cycle->log, 0,
            "API Security rate limiter using shared memory, max_per_min: %ui",
            max_per_min);
      } else {
        ngx_log_error(NGX_LOG_WARN, ngx_cycle->log, 0,
...
std::optional<HashedLowercaseStringView> Library::custom_ip_header() {
...
bool Library::api_security_should_sample() noexcept {
  return shared_api_security_limiter_ ? shared_api_security_limiter_->allow()
                                      : false;
}
