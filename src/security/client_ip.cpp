...
struct ipaddr {
  ...
  static std::optional<ipaddr> from_string(std::string_view addr_sv, int af_hint = 0);
};
...
class header_proc_def {
    public:
    using extract_func_t = extract_res (*)(const ngx_table_elt_t& value, ipaddr &out);

    constexpr header_proc_def(std::string_view key, extract_func_t parse_func)
        : lc_key_{key}, lc_key_hash_{dns::ngx_hash_ce(key)}, parse_func_{parse_func} {}

    std::string_view lc_key_;
    ngx_uint_t lc_key_hash_;
    extract_func_t parse_func_;
};
extract_res parse_multiple_maybe_port(const ngx_table_elt_t& value, ipaddr &out);
extract_res parse_multiple_maybe_port_sv(std::string_view sv, ipaddr &out);
extract_res parse_forwarded(const ngx_table_elt_t& value, ipaddr &out);
...
extract_res parse_multiple_maybe_port(const ngx_table_elt_t &value,
                                      ipaddr &out) {
...
extract_res parse_multiple_maybe_port_sv(std::string_view value_sv,
    ...
    do {
        ...
        const char *comma = reinterpret_cast<const char *>(
            std::memchr(value, ',', end - value));
        const char *end_cur = comma ? comma : end;
        std::string_view cur_str{value,
                                 static_cast<std::size_t>(end_cur - value)};
    ...
    while (r < end) {
        switch (state) {  // NOLINT
            ...
            case value_token: {
                ...
                if (consider_value) {
                    std::string_view cur_str{
                        start, static_cast<std::size_t>(token_end - start)};
            ...
            case value_quoted:
                if (*r == '"') {
                    if (consider_value) {
                      ...
                      // unescape them
                      std::string_view cur_str{
                          start, static_cast<std::size_t>(r - start)};
                      std::optional<ipaddr> maybe_cur =
                          parse_ip_address_maybe_port_pair(cur_str);
                      if (maybe_cur) {
                        if (!maybe_cur->is_private()) {
...
std::optional<ipaddr> parse_ip_address_maybe_port_pair(std::string_view addr_sv) {
    if (addr_sv.empty()) {
        return std::nullopt;
    }
    if (addr_sv[0] == '[') {  // ipv6
...
std::string ClientIp::normalize_configured_header(std::string_view config) {
...
std::optional<std::string> ClientIp::resolve() const {
    if (configured_header_) {
        std::optional<ngx_table_elt_t> maybe_header = get_request_header(
            request_.headers_in.headers, configured_header_->sv,
