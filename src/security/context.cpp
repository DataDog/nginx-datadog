...
class PolFinalWafCtx : public PolTaskCtx<PolFinalWafCtx> {
 ...
 public:
  ...
  // to call if submission fails
  void manual_complete() noexcept { complete(); }
};
...
ngx_int_t Context::do_header_filter(ngx_http_request_t &request,
  ...
  // No body (or we're ignoring it), start the WAF
  // Afterwards, we transition to PENDING_WAF_END (or AFTER_RUN_WAF_END in we
    ...
    if (start_waf) {
      ...
      if (task_ctx.submit(conf->waf_pool)) {
        ...
        ngx_log_error(NGX_LOG_NOTICE, request.connection->log, 0,
                      ...
                      "manually calling task's completion handler");
        task_ctx.manual_complete();
      }
    }
