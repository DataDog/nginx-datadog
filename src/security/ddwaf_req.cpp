...
DdwafContext::WafRunResult DdwafContext::run(ngx_log_t &log, ddwaf_object &persistent_data) {
  ...
  if (code == DDWAF_MATCH) {
    std::optional<ddwaf_map_obj> actions_arr{iresult.get_opt<ddwaf_map_obj>("actions"sv)};
    if (actions_arr) {
  ...
  if (maybe_attributes) {
    ...
    for (auto&& a: attributes) {
      handle_attribute(log,
          a, [&a, &waf_result](std::string_view json) {
