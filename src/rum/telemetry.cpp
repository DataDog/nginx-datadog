...
void increment_counter(const datadog::telemetry::Counter& counter,
  ...
  std::vector<std::string> specific_tags_vec(specific_tags.begin(),
                                             specific_tags.end());
  increment_counter(counter, specific_tags_vec);
}

datadog::telemetry::Counter injection_skipped = {"injection.skipped", "rum",
                                                 true};

datadog::telemetry::Counter injection_succeed = {"injection.succeed", "rum",
                                                 true};

datadog::telemetry::Counter injection_failed = {"injection.failed", "rum",
                                                true};
