#!/usr/bin/env python3
"""
This script automates the process of preparing an NGINX source directory 
that is compatible with the ingress-nginx controller.
...
def prepare(args) -> int:
    """
    Automates the process of preparing an NGINX source directory
    that is compatible with the ingress-nginx controller.
    ...
    with tempfile.TemporaryDirectory() as work_dir:
        ...
        # NOTE(@dmehala): Release after v1.11.2 renamed nginx-1.25 to nginx
        # PR: <https://github.com/kubernetes/ingress-nginx/pull/12031>
        patches_path = f"{ingress_nginx_path}/images/nginx-1.25/rootfs/patches"
...
def build_init_container(args) -> int:
    dockerfile_path = os.path.join(
        PROJECT_DIR, "injection", "ingress-nginx", "Dockerfile"
    )
    run_cmd(
        f"docker build --progress=plain --build-arg MODULE_PATH={args.module_path} --tag {args.image_name} -f {dockerfile_path} {PROJECT_DIR}"
    )
