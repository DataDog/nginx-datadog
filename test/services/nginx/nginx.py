...
class NgxChainPrinter(gdb.Command):
    ...
    def __init__(self):
        super(NgxChainPrinter, self).__init__("ngx_chain_print",
                                              gdb.COMMAND_USER)
    ...
    def invoke(self, arg, from_tty):
        ...
        while chain_ptr:
            ...
            if buf_ptr:
                ...
                flag_names = [
                    "temporary", "memory", "mmap", "recycled", "in_file",
                    "flush", "sync", "last_buf", "last_in_chain",
                    "last_shadow", "temp_file"
                ]
                flags_set = []
                ...
                data = pos.string(
                    length=unread_size) if unread_size > 0 else ""

                print(
                    f"  buf[0x{buf_addr:x}, size={buf_size}] unread[{unread_size}]"
                )
                if flags_set:
...
class NgxHeadersPrinter(gdb.Command):
    ...
    def __init__(self):
        super(NgxHeadersPrinter, self).__init__("ngx_headers_print",
                                                gdb.COMMAND_USER)
            ...
            if nelts > 0 and elts:
                ...
                for i in range(nelts):
                    ...
                    try:
                        ...
                        key_str = key_data.string(
                            length=key_len) if key_len > 0 else ""
                        value_str = value_data.string(
                            length=value_len) if value_len > 0 else ""

                        hash_val = int(header['hash'])

                        print(
                            f"    [{i}] header[0x{header_addr:x}] hash=0x{hash_val:x}"
                        )
