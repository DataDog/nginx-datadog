...
class MockClock {
 public:
  ...
  static void set_time(time_point time) { current_time_ = time; }

  static void advance_time(duration d) { current_time_ += d; }

  static void reset() { current_time_ = time_point{}; }
...
auto create_test_limiter(std::uint32_t max_per_min) {
  ...
  dnsec::SharedLimiter<RefreshesPerMin, MockClock>::initialize_shared_state(
      state, max_per_min);
...
TEST_CASE("Limiter token refresh behavior", "[limiter]") {
  ...
  SECTION("Tokens refresh after time passes") {
    auto limiter =
        create_test_limiter<100>(100);  // 100 tokens per minute, 100 refreshes
                                        // per minute = 1 token per 0.6 seconds
  ...
  SECTION("Partial token consumption and refresh") {
    ...
    // Advance time by 6 seconds (should add 10 tokens with 100 refreshes per
    // minute)
...
TEST_CASE("Limiter refresh amount distribution", "[limiter]") {
  ...
  SECTION(
      "Even distribution when max_per_min is divisible by refreshes_per_min") {
    auto limiter = create_test_limiter<10>(
        100);  // 100 tokens per minute, 10 refreshes = 10 tokens per refresh
    ...
    for (int refresh = 0; refresh < 10; ++refresh) {
      MockClock::advance_time(
          std::chrono::seconds(6));  // 60s / 10 refreshes = 6s per refresh
  ...
  SECTION(
      "Uneven distribution when max_per_min is not divisible by "
      "refreshes_per_min") {
    auto limiter =
        create_test_limiter<10>(107);  // 107 tokens per minute, 10 refreshes
    // Should distribute as: 7 refreshes get 10 tokens, 3 refreshes get 11
    // tokens
