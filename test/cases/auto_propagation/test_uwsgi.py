...
class TestUWSGI(case.TestCase):
    @skipIf(os.getenv("NGINX_FLAVOR", "") == "ingress-nginx", "skip because flavor")
    ...
    def test_disabled_at_location(self):
        return self.run_test(
            "./conf/uwsgi_disabled_at_location.conf", should_propagate=False
        )

    @skipIf(os.getenv("NGINX_FLAVOR", "") == "ingress-nginx", "skip because flavor")
    def test_disabled_at_server(self):
        return self.run_test(
            "./conf/uwsgi_disabled_at_server.conf", should_propagate=False
        )

    @skipIf(os.getenv("NGINX_FLAVOR", "") == "ingress-nginx", "skip because flavor")
    def test_disabled_at_http(self):
        return self.run_test(
            "./conf/uwsgi_disabled_at_http.conf", should_propagate=False
        )

    @skipIf(os.getenv("NGINX_FLAVOR", "") == "ingress-nginx", "skip because flavor")
    def test_without_module(self):
        return self.run_test("./conf/uwsgi_without_module.conf", should_propagate=False)
