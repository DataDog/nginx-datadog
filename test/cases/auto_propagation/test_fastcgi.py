...
class TestFastCGI(case.TestCase):
    def test_auto_propagation(self):
        ...
        self.assertEqual("not-hidden-by-autoinjection", headers["server-block-header"])
    ...
    def test_disabled_at_location(self):
        return self.run_test(
            "./conf/fastcgi_disabled_at_location.conf", should_propagate=False
        )

    def test_disabled_at_server(self):
        return self.run_test(
            "./conf/fastcgi_disabled_at_server.conf", should_propagate=False
        )

    def test_disabled_at_http(self):
        return self.run_test(
            "./conf/fastcgi_disabled_at_http.conf", should_propagate=False
        )

    def test_without_module(self):
        return self.run_test(
            "./conf/fastcgi_without_module.conf", should_propagate=False
        )
