...
class TestApiSecurityAttributes(case.TestCase):
    ...
    def test_api_security_enabled_with_query_params(self):
        ...
        spans = self.do_request_and_get_spans('/http?param1=value1&param2=value2')
        ...
        decoded_query = self.decode_attribute_value(api_security_tags['_dd.appsec.s.req.query'])
    ...
    def test_api_security_enabled_with_json_body(self):
        ...
        spans = self.do_request_and_get_spans(
            '/http', method='POST', headers=headers, req_body=json_body)
        api_security_tags = self.find_api_security_tags(spans)
        ...
        decoded_body = self.decode_attribute_value(api_security_tags['_dd.appsec.s.req.body'])
    ...
    def test_api_security_enabled_with_headers(self):
        ...
        decoded_headers = self.decode_attribute_value(api_security_tags['_dd.appsec.s.req.headers'])
    ...
    def test_api_security_enabled_with_cookies(self):
        ...
        decoded_cookies = self.decode_attribute_value(api_security_tags['_dd.appsec.s.req.cookies'])
            ...
            "items": [
                {
                    ...
                    "metadata": {
                        ...
                        "properties": {f"prop_{k}": f"value_{k}" for k in range(20)}
                    }
                }
                for i in range(10)
            ]
    ...
    def test_response_body_extraction(self):
        ...
        spans = self.do_request_and_get_spans('/http/response_body_test/?trigger=safe&format=json')
        api_security_tags = self.find_api_security_tags(spans)
        ...
        decoded_body = self.decode_attribute_value(api_security_tags['_dd.appsec.s.res.body'])
    ...
    def test_response_headers_extraction(self):
        ...
        decoded_headers = self.decode_attribute_value(api_security_tags['_dd.appsec.s.res.headers'])
        self.assertIsInstance(decoded_headers, (dict, list))
