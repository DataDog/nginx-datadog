...
class TestSecAddresses(case.TestCase):
    ...
    def test_value_third_occurrence(self):
        result = self.do_request_query('key=another+value&key=yet+another+value&key=matched+value')
        self.assertEqual(result['triggers'][0]['rule_matches'][0]['parameters'][0]['value'], 'matched value')
    ...
    def test_cookie_trimming(self):
        result = self.do_request_headers({'Cookie': 'a=b; key=  matched+value '})
        self.assertEqual(result['triggers'][0]['rule_matches'][0]['parameters'][0]['value'], 'matched value')

    def test_cookie_multiple(self):
        result = self.do_request_headers([('Cookie', 'a=b'), ('Cookie', 'key=matched+value')])
        self.assertEqual(result['triggers'][0]['rule_matches'][0]['parameters'][0]['value'], 'matched value')

    def test_cookie_multiple2(self):
        result = self.do_request_headers([('Cookie', 'key=matched+value'), ('Cookie', 'a=b')])
        self.assertEqual(result['triggers'][0]['rule_matches'][0]['parameters'][0]['value'], 'matched value')
    ...
    def test_client_ip_1(self):
        ...
        self.assertEqual(result['triggers'][0]['rule_matches'][0]['parameters'][0]['value'], '1.2.3.4')

    def test_client_ip_2(self):
        result = self.do_request_headers({'x-real-ip': '8.8.8.8, 1.2.3.4'})
        self.assertEqual(result['triggers'][0]['rule_matches'][0]['parameters'][0]['value'], '8.8.8.8')

    def test_client_ip_3(self):
        result = self.do_request_headers([('x-real-ip', '127.0.0.1'), ('x-real-ip', '1.2.3.4')])
        self.assertEqual(result['triggers'][0]['rule_matches'][0]['parameters'][0]['value'], '1.2.3.4')

    def test_client_ip_4(self):
        result = self.do_request_headers([('x-real-ip', '8.8.8.8'), ('x-real-ip', '1.2.3.4')])
        self.assertEqual(result['triggers'][0]['rule_matches'][0]['parameters'][0]['value'], '8.8.8.8')
    ...
    def test_client_ip_8(self):
        result = self.do_request_headers({'x-forwarded': 'for=127.0.0.1, for=1.2.3.4'})
        self.assertEqual(result['triggers'][0]['rule_matches'][0]['parameters'][0]['value'], '1.2.3.4')
    ...
    def test_client_ip_13(self):
        ...
        self.assertEqual(result['triggers'][0]['rule_matches'][0]['parameters'][0]['value'], 'fe80::1')

    def test_client_ip_prio_1(self):
        result = self.do_request_headers({'x-real-ip': '8.8.8.8', 'x-forwarded-for': '1.2.3.4'})
        self.assertEqual(result['triggers'][0]['rule_matches'][0]['parameters'][0]['value'], '1.2.3.4')

    def test_client_ip_prio_2(self):
        result = self.do_request_headers({'true-client-ip': '8.8.8.8', 'x-real-ip': '1.2.3.4'})
        self.assertEqual(result['triggers'][0]['rule_matches'][0]['parameters'][0]['value'], '1.2.3.4')

    def test_client_ip_prio_3(self):
        result = self.do_request_headers({'x-client-ip': '8.8.8.8', 'true-client-ip': '1.2.3.4'})
        self.assertEqual(result['triggers'][0]['rule_matches'][0]['parameters'][0]['value'], '1.2.3.4')

    def test_client_ip_prio_4(self):
        result = self.do_request_headers({'x-forwarded': 'for=8.8.8.8', 'x-client-ip': '1.2.3.4'})
        self.assertEqual(result['triggers'][0]['rule_matches'][0]['parameters'][0]['value'], '1.2.3.4')

    def test_client_ip_prio_5(self):
        result = self.do_request_headers({'forwarded-for': '8.8.8.8', 'x-forwarded': 'for=1.2.3.4'})
        self.assertEqual(result['triggers'][0]['rule_matches'][0]['parameters'][0]['value'], '1.2.3.4')

    def test_client_ip_prio_6(self):
        result = self.do_request_headers({'x-cluster-client-ip': '8.8.8.8', 'forwarded-for': '1.2.3.4'})
        self.assertEqual(result['triggers'][0]['rule_matches'][0]['parameters'][0]['value'], '1.2.3.4')

    def test_client_ip_prio_7(self):
        result = self.do_request_headers({'fastly-client-ip': '8.8.8.8', 'x-cluster-client-ip': '1.2.3.4'})
        self.assertEqual(result['triggers'][0]['rule_matches'][0]['parameters'][0]['value'], '1.2.3.4')

    def test_client_ip_prio_8(self):
        result = self.do_request_headers({'cf-connecting-ip': '8.8.8.8', 'fastly-client-ip': '1.2.3.4'})
        self.assertEqual(result['triggers'][0]['rule_matches'][0]['parameters'][0]['value'], '1.2.3.4')

    def test_client_ip_prio_9(self):
        result = self.do_request_headers({'cf-connecting-ip': '1.2.3.4', 'cf-connecting-ipv6': '1.2.3.4'})
        self.assertEqual(result['triggers'][0]['rule_matches'][0]['parameters'][0]['value'], '1.2.3.4')
